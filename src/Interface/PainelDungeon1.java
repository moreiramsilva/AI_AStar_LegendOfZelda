...
public class PainelDungeon1 extends JPanel {

	private boolean keepRunning;
	private boolean keepBreak = true;
	...
	public PainelDungeon1() {
		// TODO Auto-generated constructor stub
	}

	public PainelDungeon1(ArrayList<Limites> movimento) {
			...
			for(int linha=0; linha<SIZE;linha++)
				for(int coluna=0; coluna<SIZE;coluna++){
					if(mapa[linha][coluna] == 999){
						color = new Color(120, 134, 107); // verde militar montanhas intransponíveis
					}
					else if(mapa[linha][coluna] == 10){
						color = new Color(240, 230, 140); // marrom areia caminhos possíveis
					}


					colors[linha][coluna] = color; // altera o valor da cor no vetor de cores
					fireTableCellUpdated(linha, coluna); // faz o update da célula selecionada

				}
			color = new Color(0, 127, 255); // cor do pingente da coragem "AZUL" 
			colors[3][13] = color;
			fireTableCellUpdated(3, 13);
	...
	private class RandomColorShadeRunnable implements Runnable {
		public void run() {
			tableModel.ColoreMapa("mapa_dg1.txt");
			int i=0;
			while (keepRunning) {
